<template>
  <div class="ref">
    <div class="ref__container">
      <div class="ref__section">
        <main-title tag="h2" titleName="Реферальная программа"></main-title>
        <div class="ref__block">
          <div class="ref__wrap wrap">
            <main-title
              v-if="this.viewportWidth < 767.98"
              tag="h3"
              titleName="Реферальный баланс"
            ></main-title>
            <div class="ref__wrap_block ref__wrap_block-adapt">
              <span class="ref__subtitle">Баланс:</span>
              <span class="ref__balance main__number-lg"
                ><span>{{ this.bitcoins.toFixed(8) }}</span> BTC</span
              >
              <p class="ref__text">
                Ваш реферальный баланс невыплаченных средств, выплата происходит
                один раз в неделю по пятницвм.
              </p>
              <span class="ref__percent" v-if="this.viewportWidth <= 767"
                >Текущий процент: <span>{{ this.percent }}%</span></span
              >
            </div>
            <div class="ref__wrap_block">
              <div class="ref__link">
                Ваша ссылка:
                <span class="copy" ref="copy"
                  >{{ this.link }}
                  <div class="ref__link-copy copy-button" @click="this.copy(1)">
                    <svg
                      class="connecting__block-copy-copy"
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        d="M15 3V6.4C15 6.96005 15 7.24008 15.109 7.45399C15.2049 7.64215 15.3578 7.79513 15.546 7.89101C15.7599 8 16.0399 8 16.6 8H20M10 8H6C4.89543 8 4 8.89543 4 10V19C4 20.1046 4.89543 21 6 21H12C13.1046 21 14 20.1046 14 19V16M16 3H13.2C12.0799 3 11.5198 3 11.092 3.21799C10.7157 3.40973 10.4097 3.71569 10.218 4.09202C10 4.51984 10 5.0799 10 6.2V12.8C10 13.9201 10 14.4802 10.218 14.908C10.4097 15.2843 10.7157 15.5903 11.092 15.782C11.5198 16 12.0799 16 13.2 16H16.8C17.9201 16 18.4802 16 18.908 15.782C19.2843 15.5903 19.5903 15.2843 19.782 14.908C20 14.4802 20 13.9201 20 12.8V7L16 3Z"
                        stroke-opacity="0.33"
                        stroke-width="2"
                        stroke-linejoin="round"
                      ></path>
                    </svg></div
                ></span>
              </div>
              <span class="ref__percent" v-if="this.viewportWidth > 767"
                >Текущий процент: <span>{{ this.percent }}%</span></span
              >
            </div>
          </div>
          <div class="ref__wrap ref__wrap-back wrap">
            <main-title
              tag="h3"
              titleName="Приглашайте людей и зарабатывайте"
            ></main-title>
            <p class="ref__text">
              Зарабатывайте до <span>0,5%</span> с майнинга приглашенных
              пользователей
            </p>
            <router-link class="main__link main__link-underline" to="/faqview"
              >Подбробнее в FAQ
            </router-link>
          </div>
        </div>
      </div>
      <div class="ref__section">
        <main-title
          class="ref__title"
          tag="h2"
          titleName="Статистика и прогноз"
        >
          <div class="ref__title_buttons">
            <button class="ref__title_button ref__title_button-active">
              Текущий месяц
            </button>
            <button v-if="this.viewportWidth > 991" class="ref__title_button">
              Прогнозируемый следующий месяц
            </button>
            <button v-else class="ref__title_button">Прогноз</button>
          </div>
        </main-title>
        <div class="ref__wrap wrap ref__wrap-statistic">
          <div class="ref__wrap_column">
            <div class="ref__wrap_wrapper">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  d="M15.29 10.29L12.51 13.07C12.3436 13.0257 12.1723 13.0022 12 13C11.4696 13 10.9609 13.2107 10.5858 13.5858C10.2107 13.9609 10 14.4696 10 15C10.0023 15.1722 10.0258 15.3435 10.07 15.51L9.29003 16.29C9.1963 16.383 9.12191 16.4936 9.07114 16.6154C9.02037 16.7373 8.99423 16.868 8.99423 17C8.99423 17.132 9.02037 17.2627 9.07114 17.3846C9.12191 17.5064 9.1963 17.617 9.29003 17.71C9.38299 17.8037 9.49359 17.8781 9.61545 17.9289C9.73731 17.9797 9.86802 18.0058 10 18.0058C10.132 18.0058 10.2627 17.9797 10.3846 17.9289C10.5065 17.8781 10.6171 17.8037 10.71 17.71L11.49 16.93C11.6565 16.9743 11.8278 16.9978 12 17C12.5305 17 13.0392 16.7893 13.4142 16.4142C13.7893 16.0391 14 15.5304 14 15C13.9978 14.8278 13.9743 14.6565 13.93 14.49L16.71 11.71C16.8983 11.5217 17.0041 11.2663 17.0041 11C17.0041 10.7337 16.8983 10.4783 16.71 10.29C16.5217 10.1017 16.2663 9.99591 16 9.99591C15.7337 9.99591 15.4783 10.1017 15.29 10.29ZM12 4C10.6868 4 9.38645 4.25866 8.1732 4.7612C6.95994 5.26375 5.85755 6.00035 4.92896 6.92893C3.0536 8.8043 2.00003 11.3478 2.00003 14C1.99511 15.9806 2.58375 17.9172 3.69003 19.56C3.83855 19.7801 4.06844 19.9322 4.32911 19.9829C4.58979 20.0335 4.8599 19.9785 5.08003 19.83C5.30016 19.6815 5.45227 19.4516 5.50291 19.1909C5.55354 18.9302 5.49855 18.6601 5.35003 18.44C4.54455 17.2355 4.08131 15.8349 4.00978 14.3877C3.93826 12.9405 4.26113 11.501 4.94392 10.223C5.62671 8.94501 6.64378 7.87643 7.88653 7.13139C9.12928 6.38634 10.5511 5.99281 12 5.99281C13.449 5.99281 14.8708 6.38634 16.1135 7.13139C17.3563 7.87643 18.3734 8.94501 19.0561 10.223C19.7389 11.501 20.0618 12.9405 19.9903 14.3877C19.9188 15.8349 19.4555 17.2355 18.65 18.44C18.5763 18.5489 18.5248 18.6714 18.4985 18.8002C18.4722 18.9291 18.4716 19.0619 18.4966 19.191C18.5217 19.3201 18.572 19.443 18.6447 19.5527C18.7173 19.6623 18.8109 19.7566 18.92 19.83C19.0856 19.9412 19.2806 20.0004 19.48 20C19.6438 20.0002 19.8052 19.9603 19.9499 19.8835C20.0946 19.8068 20.2183 19.6957 20.31 19.56C21.4163 17.9172 22.005 15.9806 22 14C22 11.3478 20.9465 8.8043 19.0711 6.92893C17.1957 5.05357 14.6522 4 12 4Z"
                />
              </svg>
            </div>
            <span class="ref__subtitle">Реферальный хешрейт</span>
            <span class="ref__value"
              ><span>{{ this.refHash }}</span> TH/s</span
            >
          </div>
          <div class="ref__wrap_column">
            <div class="ref__wrap_wrapper">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  d="M7.75778 10.7578C8.35113 10.7578 8.93115 10.5819 9.4245 10.2522C9.91784 9.92258 10.3024 9.45404 10.5294 8.90586C10.7565 8.35769 10.8159 7.75449 10.7001 7.17254C10.5844 6.5906 10.2987 6.05605 9.87911 5.63649C9.45955 5.21694 8.925 4.93121 8.34306 4.81546C7.76111 4.6997 7.15791 4.75911 6.60973 4.98618C6.06156 5.21324 5.59302 5.59776 5.26338 6.0911C4.93373 6.58445 4.75778 7.16447 4.75778 7.75781C4.75865 8.5532 5.075 9.31575 5.63742 9.87818C6.19984 10.4406 6.9624 10.7569 7.75778 10.7578ZM7.75778 6.75781C7.95557 6.75781 8.14891 6.81646 8.31335 6.92634C8.4778 7.03623 8.60598 7.1924 8.68166 7.37513C8.75735 7.55786 8.77715 7.75892 8.73857 7.9529C8.69998 8.14689 8.60474 8.32507 8.46489 8.46492C8.32504 8.60477 8.14686 8.70001 7.95287 8.7386C7.75889 8.77718 7.55783 8.75738 7.3751 8.68169C7.19237 8.60601 7.0362 8.47783 6.92632 8.31338C6.81643 8.14894 6.75778 7.9556 6.75778 7.75781C6.75796 7.49265 6.86338 7.2384 7.05087 7.0509C7.23837 6.86341 7.49262 6.75799 7.75778 6.75781ZM16.2422 13.2422C15.6488 13.2422 15.0688 13.4181 14.5755 13.7478C14.0821 14.0774 13.6976 14.546 13.4705 15.0941C13.2435 15.6423 13.1841 16.2455 13.2998 16.8275C13.4156 17.4094 13.7013 17.944 14.1208 18.3635C14.5404 18.7831 15.075 19.0688 15.6569 19.1845C16.2388 19.3003 16.842 19.2409 17.3902 19.0138C17.9384 18.7868 18.4069 18.4023 18.7366 17.9089C19.0662 17.4156 19.2422 16.8355 19.2422 16.2422C19.2413 15.4468 18.9249 14.6843 18.3625 14.1218C17.8001 13.5594 17.0375 13.2431 16.2422 13.2422ZM16.2422 17.2422C16.0444 17.2422 15.851 17.1835 15.6866 17.0737C15.5221 16.9638 15.394 16.8076 15.3183 16.6249C15.2426 16.4422 15.2228 16.2411 15.2614 16.0471C15.3 15.8531 15.3952 15.6749 15.5351 15.5351C15.6749 15.3952 15.8531 15.3 16.0471 15.2614C16.2411 15.2228 16.4421 15.2426 16.6248 15.3183C16.8076 15.394 16.9638 15.5222 17.0736 15.6866C17.1835 15.8511 17.2422 16.0444 17.2422 16.2422C17.242 16.5074 17.1366 16.7616 16.9491 16.9491C16.7616 17.1366 16.5073 17.242 16.2422 17.2422ZM19.707 4.293C19.6141 4.20012 19.5039 4.12644 19.3826 4.07617C19.2613 4.0259 19.1313 4.00003 18.9999 4.00003C18.8686 4.00003 18.7386 4.0259 18.6173 4.07617C18.496 4.12644 18.3857 4.20012 18.2929 4.293L4.29291 18.293C4.19911 18.3856 4.12455 18.4959 4.07353 18.6175C4.0225 18.739 3.99602 18.8695 3.99561 19.0013C3.9952 19.1332 4.02086 19.2638 4.07112 19.3857C4.12139 19.5075 4.19526 19.6183 4.28848 19.7115C4.38171 19.8047 4.49245 19.8786 4.61433 19.9288C4.73621 19.9791 4.86683 20.0048 4.99866 20.0043C5.1305 20.0039 5.26095 19.9774 5.38251 19.9264C5.50407 19.8754 5.61434 19.8008 5.70697 19.707L19.707 5.707C19.7998 5.61417 19.8735 5.50396 19.9238 5.38265C19.974 5.26134 19.9999 5.13131 19.9999 5C19.9999 4.86869 19.974 4.73867 19.9238 4.61736C19.8735 4.49605 19.7998 4.38583 19.707 4.293Z"
                />
              </svg>
            </div>
            <span class="ref__subtitle">Процент вознаграждения</span>
            <span class="ref__value"
              ><span>{{ this.percent }}</span> %</span
            >
          </div>
          <div class="ref__wrap_column" v-if="this.viewportWidth > 767">
            <span class="ref__subtitle">Количество активных рефералов</span>
            <span class="ref__value"><span>1</span></span>
          </div>
          <div class="ref__wrap_column">
            <div class="ref__wrap_wrapper">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  d="M12 12C11.4067 12 10.8266 12.1759 10.3333 12.5056C9.83994 12.8352 9.45542 13.3038 9.22836 13.8519C9.0013 14.4001 8.94189 15.0033 9.05764 15.5853C9.1734 16.1672 9.45912 16.7018 9.87868 17.1213C10.2982 17.5409 10.8328 17.8266 11.4147 17.9424C11.9967 18.0581 12.5999 17.9987 13.1481 17.7716C13.6962 17.5446 14.1648 17.1601 14.4944 16.6667C14.8241 16.1734 15 15.5933 15 15C15 14.2044 14.6839 13.4413 14.1213 12.8787C13.5587 12.3161 12.7956 12 12 12ZM12 16C11.8022 16 11.6089 15.9414 11.4444 15.8315C11.28 15.7216 11.1518 15.5654 11.0761 15.3827C11.0004 15.2 10.9806 14.9989 11.0192 14.8049C11.0578 14.6109 11.153 14.4327 11.2929 14.2929C11.4327 14.153 11.6109 14.0578 11.8049 14.0192C11.9989 13.9806 12.2 14.0004 12.3827 14.0761C12.5654 14.1518 12.7216 14.28 12.8315 14.4444C12.9414 14.6089 13 14.8022 13 15C13 15.2652 12.8946 15.5196 12.7071 15.7071C12.5196 15.8946 12.2652 16 12 16ZM11.29 9.71C11.3851 9.80104 11.4972 9.87241 11.62 9.92C11.7397 9.97291 11.8691 10.0002 12 10.0002C12.1309 10.0002 12.2603 9.97291 12.38 9.92C12.5028 9.87241 12.6149 9.80104 12.71 9.71L15 7.46C15.1936 7.26639 15.3024 7.0038 15.3024 6.73C15.3024 6.4562 15.1936 6.19361 15 6C14.8064 5.80639 14.5438 5.69762 14.27 5.69762C13.9962 5.69762 13.7336 5.80639 13.54 6L13 6.59V3C13 2.73478 12.8946 2.48043 12.7071 2.29289C12.5196 2.10536 12.2652 2 12 2C11.7348 2 11.4804 2.10536 11.2929 2.29289C11.1054 2.48043 11 2.73478 11 3V6.59L10.46 6C10.2664 5.80639 10.0038 5.69762 9.73 5.69762C9.4562 5.69762 9.19361 5.80639 9 6C8.80639 6.19361 8.69762 6.4562 8.69762 6.73C8.69762 7.0038 8.80639 7.26639 9 7.46L11.29 9.71ZM19 15C19 14.8022 18.9414 14.6089 18.8315 14.4444C18.7216 14.28 18.5654 14.1518 18.3827 14.0761C18.2 14.0004 17.9989 13.9806 17.8049 14.0192C17.6109 14.0578 17.4327 14.153 17.2929 14.2929C17.153 14.4327 17.0578 14.6109 17.0192 14.8049C16.9806 14.9989 17.0004 15.2 17.0761 15.3827C17.1518 15.5654 17.28 15.7216 17.4444 15.8315C17.6089 15.9414 17.8022 16 18 16C18.2652 16 18.5196 15.8946 18.7071 15.7071C18.8946 15.5196 19 15.2652 19 15ZM20 8H17C16.7348 8 16.4804 8.10536 16.2929 8.29289C16.1054 8.48043 16 8.73478 16 9C16 9.26522 16.1054 9.51957 16.2929 9.70711C16.4804 9.89464 16.7348 10 17 10H20C20.2652 10 20.5196 10.1054 20.7071 10.2929C20.8946 10.4804 21 10.7348 21 11V19C21 19.2652 20.8946 19.5196 20.7071 19.7071C20.5196 19.8946 20.2652 20 20 20H4C3.73478 20 3.48043 19.8946 3.29289 19.7071C3.10536 19.5196 3 19.2652 3 19V11C3 10.7348 3.10536 10.4804 3.29289 10.2929C3.48043 10.1054 3.73478 10 4 10H7C7.26522 10 7.51957 9.89464 7.70711 9.70711C7.89464 9.51957 8 9.26522 8 9C8 8.73478 7.89464 8.48043 7.70711 8.29289C7.51957 8.10536 7.26522 8 7 8H4C3.20435 8 2.44129 8.31607 1.87868 8.87868C1.31607 9.44129 1 10.2044 1 11V19C1 19.7956 1.31607 20.5587 1.87868 21.1213C2.44129 21.6839 3.20435 22 4 22H20C20.7956 22 21.5587 21.6839 22.1213 21.1213C22.6839 20.5587 23 19.7956 23 19V11C23 10.2044 22.6839 9.44129 22.1213 8.87868C21.5587 8.31607 20.7956 8 20 8ZM5 15C5 15.1978 5.05865 15.3911 5.16853 15.5556C5.27841 15.72 5.43459 15.8482 5.61732 15.9239C5.80004 15.9996 6.00111 16.0194 6.19509 15.9808C6.38907 15.9422 6.56725 15.847 6.70711 15.7071C6.84696 15.5673 6.9422 15.3891 6.98079 15.1951C7.01937 15.0011 6.99957 14.8 6.92388 14.6173C6.84819 14.4346 6.72002 14.2784 6.55557 14.1685C6.39112 14.0586 6.19778 14 6 14C5.73478 14 5.48043 14.1054 5.29289 14.2929C5.10536 14.4804 5 14.7348 5 15Z"
                />
              </svg>
            </div>
            <span class="ref__subtitle">Доход за месяц</span>
            <span class="ref__value"><span>0.00000681</span> BTC</span>
          </div>
        </div>
      </div>
      <div class="ref__section">
        <main-title
          tag="h2"
          titleName="Реферальные начисления"
          class="ref__title"
        >
          <div
            v-if="this.viewportWidth > 767"
            @click="this.dateFilterAcc"
            ref="filterAcc"
            class="ref__filter"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                d="M3 10H21M7 3V5M17 3V5M6.2 21H17.8C18.9201 21 19.4802 21 19.908 20.782C20.2843 20.5903 20.5903 20.2843 20.782 19.908C21 19.4802 21 18.9201 21 17.8V8.2C21 7.07989 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V17.8C3 18.9201 3 19.4802 3.21799 19.908C3.40973 20.2843 3.71569 20.5903 4.09202 20.782C4.51984 21 5.07989 21 6.2 21Z"
                stroke-opacity="0.62"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
            Фильтровать по дате
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                d="M18 10L12 16L6 10"
                stroke-opacity="0.62"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>
          <div class="ref__filter" v-else>
            <div class="ref_filter">
              <div class="ref_filter_label main__label">Дата</div>
              <main-date placeholder="За все время"></main-date>
            </div>
            <blue-button class="ref__button"
              ><a href="">Выгрузить</a></blue-button
            >
          </div>
        </main-title>
        <main-slider
          class="ref__slider"
          :table="this.refAccrualsInfo"
        ></main-slider>
      </div>
      <div class="ref__section">
        <main-title tag="h2" titleName="Реферальные выплаты" class="ref__title">
          <div
            v-if="this.viewportWidth > 767"
            @click="this.dateFilterPay"
            ref="filterPay"
            class="ref__filter"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                d="M3 10H21M7 3V5M17 3V5M6.2 21H17.8C18.9201 21 19.4802 21 19.908 20.782C20.2843 20.5903 20.5903 20.2843 20.782 19.908C21 19.4802 21 18.9201 21 17.8V8.2C21 7.07989 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V17.8C3 18.9201 3 19.4802 3.21799 19.908C3.40973 20.2843 3.71569 20.5903 4.09202 20.782C4.51984 21 5.07989 21 6.2 21Z"
                stroke-opacity="0.62"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
            Фильтровать по дате
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                d="M18 10L12 16L6 10"
                stroke-opacity="0.62"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>
          <div v-else class="ref__filter">
            <div class="ref_filter">
              <div class="ref_filter_label main__label">Дата</div>
              <main-date placeholder="За все время"></main-date>
            </div>
            <blue-button class="ref__button"
              ><a href="">Выгрузить</a></blue-button
            >
          </div>
        </main-title>
        <main-slider
          class="ref__slider"
          :table="this.refPaymentInfo"
        ></main-slider>
      </div>
      <div class="ref__section">
        <main-title
          v-if="this.viewportWidth > 991.98"
          tag="h2"
          titleName="Список приглашаемых пользователей"
          class="ref__title"
        ></main-title>
        <main-title
          v-else
          tag="h2"
          titleName="Пользователи"
          class="ref__title"
        ></main-title>
        <main-slider class="ref__slider" :table="this.refList"></main-slider>
      </div>
    </div>
  </div>
</template>
<script>
import MainSlider from "@/components/account/MainSlider";

export default {
  components: { MainSlider },
  data() {
    return {
      date: {},
      date1: {},
      viewportWidth: 0,
      refHash: 137.13,
      bitcoins: 0,
      link: "https://allbtc.com/pool?r=258656424",
      isFilteredAcc: false,
      isFilteredPay: false,
      refAccrualsInfo: {
        titles: ["Дата", "Сумма"],
        rows: [
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
        ],
      },
      refPaymentInfo: {
        titles: ["Дата", "Сумма"],
        rows: [
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
          {
            date: "29.11.2022",
            time: "15:10:45",
            BTC: 0,
          },
        ],
      },
      refList: {
        titles: [
          "Элекстронная почта",
          "Текущий хешрейт",
          "Средний хешрейт за текущий месяц",
          "Дата регистрации",
        ],
        shortTitles: ["Почта", "Текущий хешрейт"],
        rows: [
          {
            email: "р*******@gmail.com",
            hash: 137.13,
            middleHash: 137.13,
            date: "29.11.2022",
            time: "15:10:45",
          },
          {
            email: "р*******@gmail.com",
            hash: 137.13,
            middleHash: 137.13,
            date: "29.11.2022",
            time: "15:10:45",
          },
          {
            email: "р*******@gmail.com",
            hash: 137.13,
            middleHash: 137.13,
            date: "29.11.2022",
            time: "15:10:45",
          },
        ],
      },
    };
  },
  methods: {
    copy() {
      navigator.clipboard.writeText(this.link);
      this.$refs.copy.classList.add("active");
      setTimeout(() => {
        this.$refs.copy.classList.remove("active");
      }, 1000);
    },
    dateFilterAcc() {
      if (!this.isFilteredAcc) {
        this.isFilteredAcc = true;
        this.$refs.filterAcc.classList.add("active");
      } else {
        this.isFilteredAcc = false;
        this.$refs.filterAcc.classList.remove("active");
      }
    },
    dateFilterPay() {
      if (!this.isFilteredPay) {
        this.isFilteredPay = true;
        this.$refs.filterPay.classList.add("active");
      } else {
        this.isFilteredPay = false;
        this.$refs.filterPay.classList.remove("active");
      }
    },
    handleResize() {
      this.viewportWidth = window.innerWidth;
    },
    iconRemover() {
      this.$refs.icon.style.display = "none";
    },
  },
  computed: {
    percent() {
      return 0.05;
    },
  },
  created() {
    window.addEventListener("resize", this.handleResize);
    this.handleResize();
  },
  mounted() {
    document.title = "Реферальная программа";
  },
};
</script>
<style lang="scss" scoped>
.ref {
  width: 100%;

  .main__link {
    max-width: 30%;
    @media (max-width: 479.98px) {
      max-width: 50%;
    }
  }

  .title {
    margin: 49px 0 24px;
    @media (max-width: 767.98px) {
      margin: 28px 0 24px;
      font-size: 24px;
      line-height: 30px;
      color: #000034;
    }
  }

  &__section {
    &:last-child {
      .ref__title {
        @media (max-width: 767.98px) {
          grid-template-rows: auto;
          grid-template-columns: auto;
        }
      }
    }

    &:nth-child(2) {
      .ref__title {
        gap: 40px 18px;
      }
    }

    &:not(:last-child) {
      margin-bottom: 64px;
      @media (max-width: 991.98px) {
        margin-bottom: 20px;
      }
    }

    &:not(:first-child) {
      @media (max-width: 991.98px) {
        padding: 24px;
        background: rgba(255, 255, 255, 0.29);
        border-radius: 21px;
        display: flex;
        flex-direction: column;
        gap: 20px;
        width: 100%;
        .ref__wrap {
          background-color: transparent;
          width: 100%;
          margin: 0;
          padding: 0;
          gap: 6px;
        }
      }
      @media (max-width: 767.98px) {
        margin: 0 -15px 20px;
        width: calc(100% + 30px);
      }
      @media (max-width: 479.98px) {
        margin: 0 -20px 20px;
        width: calc(100% + 40px);
      }
    }
  }

  &__link-copy {
    @media (max-width: 767.98px) {
      transform: translateY(-50%);
    }
    @media (max-width: 479.98px) {
      transform: translateY(-58%);
    }
  }

  &__title {
    width: 100%;
    display: inline-flex;
    align-items: center;
    margin: 0 0 24px !important;
    justify-content: space-between;
    @media (max-width: 767.98px) {
      flex-direction: column;
      justify-content: flex-start;
      align-items: flex-start;
      gap: 22px;
      margin: 0 !important;
      font-size: 21px !important;
      line-height: 26px !important;
      letter-spacing: 0.1px;
    }

    &_buttons {
      display: flex;
      gap: 24px;
      position: relative;
      @media (max-width: 767.98px) {
        width: 100%;
        &:before {
          content: "";
          width: 100%;
          height: 1px;
          background-color: #d7d8d9;
          position: absolute;
          top: -20px;
          left: 0;
        }
      }
      @media (max-width: 479.98px) {
        gap: 1px;
      }
    }

    &_button {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      height: 32px;
      padding: 6px 12px;
      border-radius: 16px;
      background-color: transparent;
      font-size: 17px;
      line-height: 20px;
      color: #99acd3;
      @media (max-width: 767.98px) {
        color: #818c99;
        font-size: 14px;
        line-height: 16px;
        padding: 6px 20px;
        height: 28px;
        border-radius: 8px;
      }

      &-active {
        color: #181847;
        background-color: #fff;
        @media (max-width: 767.98px) {
          color: #000000;
        }
      }
    }
  }

  &__filter {
    cursor: pointer;
    font-style: normal;
    font-size: 18px;
    line-height: 26px;
    color: rgba(0, 0, 0, 0.62);
    display: inline-flex;
    align-items: center;
    gap: 8px;
    transition: all 0.3s ease 0s;
    @media (max-width: 767.98px) {
      width: 100%;
      gap: 20px;
      display: grid;
      grid-template-rows: 1fr;
      grid-template-columns: repeat(2, 1fr);
      .blue-button {
        margin-top: auto;
        grid-column-start: 2;
        min-height: 48px;
        @media (max-width: 479.98px) {
          min-height: 34px;
        }
      }
      .ref_filter,
      .blue-button {
        width: 100%;
      }
      .ref_filter {
        grid-column-start: 1;

        &_label {
          margin-bottom: 6px;
          font-size: 14px;
          font-weight: 400;
          line-height: 20px;
          color: #818c99;
        }
      }
    }

    svg {
      width: 24px;
      height: 24px;
      stroke: rgba(0, 0, 0, 0.62);
      transition: all 0.3s ease 0s;
    }

    &.active {
      color: #4182ec;

      svg {
        stroke: #4182ec;

        &:last-child {
          transform: rotate(180deg);
        }
      }
    }
  }

  // .ref__block
  &__block {
    width: 100%;
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    margin-top: 24px;
    gap: 32px;
    margin-bottom: 32px;
    @media (max-width: 1270px) {
      grid-template-columns: 1fr;
      gap: 20px;
      margin: 0;
    }

    .ref__wrap {
      @media (min-width: 1270px) {
        min-height: 315px;
      }
      margin-bottom: 0 !important;
      gap: 22px;

      .title {
        margin: 0;

        &:after {
          content: none;
        }
      }
    }
  }

  // .ref__wrap
  &__wrap {
    @media (min-width: 1270.98px) {
      padding: 24px;
    }
    flex-direction: column;

    &-back {
      background: rgba(255, 255, 255, 0.29)
        url("../../assets/img/ref_img_back.png") no-repeat right center;
      @media (max-width: 1270px) {
        order: -1;
      }
      @media (max-width: 767.98px) {
        min-height: 315px;
        gap: 6px;
      }
      @media (max-width: 479.98px) {
        background: rgba(255, 255, 255, 0.29)
          url("../../assets/img/ref_img_back_mobile.png") no-repeat right bottom;
      }

      .ref__text {
        max-width: 280px;
        @media (max-width: 767.98px) {
          margin: 0;
        }
      }

      .title {
        max-width: 265px;
        margin-bottom: 16px;
        @media (max-width: 767.98px) {
          font-size: 20px;
          line-height: 25px;
          margin: 0 !important;
          &:after {
            content: none;
          }
        }
      }
    }

    &-statistic {
      display: flex;
      width: 100%;
      justify-content: space-between;
      align-items: flex-start;
      padding: 24px;
      flex-direction: row;
      @media (max-width: 767.98px) {
        flex-direction: column;
      }
    }

    &_column {
      display: flex;
      flex-direction: column;
      gap: 4px;
      @media (max-width: 767.98px) {
        gap: 2px;
        padding: 10px 10px 10px 56px;
        background: #ffffff;
        border-radius: 12px;
        width: 100%;
        position: relative;
      }

      .ref__subtitle {
        font-size: 16px;
        line-height: 23px;
        @media (max-width: 767.98px) {
          font-size: 14px;
          line-height: 20px;
        }
      }
    }

    &_wrapper {
      @media (min-width: 767.98px) {
        display: none;
      }
      position: absolute;
      left: 10px;
      top: 10px;
      width: 36px;
      height: 36px;
      display: inline-flex;
      align-items: center;
      justify-content: center;
      background: #f4f6f9;
      border: 0.5px solid rgba(0, 0, 0, 0.04);
      border-radius: 8px;

      svg {
        fill: #818c99;
        width: 24px;
        height: 24px;
      }
    }

    // .ref__wrap_block
    &_block {
      padding: 16px 16px 24px;
      background-color: #fff;
      border-radius: 21px;
      display: flex;
      flex-direction: column;
      gap: 24px;
      @media (max-width: 1270px) {
        align-items: center;
        flex-direction: row;
      }
      @media (max-width: 991.98px) {
        flex-direction: column;
        align-items: flex-start;
      }
      @media (max-width: 767.98px) {
        padding: 0;
        gap: 0;
        background-color: transparent;
      }

      &-adapt {
        @media (min-width: 768px) {
          padding: 0;
          gap: 0;
          background-color: transparent;
        }
        @media (max-width: 1270px) {
          align-items: flex-start;
          flex-direction: column;
        }
        @media (max-width: 767.98px) {
          padding: 4px 10px;
          background-color: #fff;
          border-radius: 8px;
          border: 0.5px solid rgba(0, 0, 0, 0.08);
          .ref__text {
            font-size: 12px;
            margin: 0 0 13px;
            position: relative;
            color: #818c99;
            line-height: 17px;

            &:after {
              content: "";
              height: 1px;
              width: 100%;
              position: absolute;
              bottom: -6.5px;
              left: 0;
              background-color: #d7d8d9;
            }
          }
        }
      }
    }
  }

  &__value {
    font-weight: 500;
    font-size: 28px;
    line-height: 40px;
    color: #0000349e;
    @media (max-width: 767.98px) {
      font-size: 14px;
      line-height: 23px;
      color: #000034;
      font-weight: 500;
    }
    @media (max-width: 479.98px) {
      font-size: 12px;
      line-height: 17px;
    }

    span {
      font-weight: 700;
      color: #000034;
      @media (max-width: 767.98px) {
        font-weight: 500;
      }
    }
  }

  // .ref__subtitle
  &__subtitle {
    font-weight: 500;
    font-size: 18px;
    line-height: 26px;
    color: rgba(0, 0, 0, 0.62);
    display: inline-flex;
    align-items: center;
    @media (max-width: 767.98px) {
      font-size: 14px;
      line-height: 23px;
      font-weight: 400;
    }
    @media (max-width: 479.98px) {
      font-size: 12px;
      line-height: 17px;
    }
  }

  // .ref__balance
  &__balance {
    margin: 1px 0 8px;
    @media (max-width: 767.98px) {
      margin: 0;
    }
  }

  // .ref__text
  &__text {
    margin-bottom: 24px;
    font-weight: 400;
    font-size: 13px;
    line-height: 18px;
    color: rgba(0, 0, 0, 0.62);

    span {
      font-weight: 700;
    }

    @media (max-width: 767.98px) {
      font-size: 14px;
      line-height: 20px;
      margin: 6px 0;
      color: #818c99;
    }
  }

  // .ref__link
  &__link {
    font-weight: 500;
    font-size: 18px;
    line-height: 26px;
    color: rgba(0, 0, 0, 0.62);
    display: inline-flex;
    align-items: center;
    white-space: nowrap;
    width: 100%;
    @media (max-width: 767.98px) {
      flex-direction: column;
      align-items: flex-start;
      width: 100%;
      color: #818c99;
      font-size: 12px;
      line-height: 17px;
      height: auto;
      font-weight: 400;
      gap: 6px;
    }

    span {
      position: relative;
      margin-left: 8px;
      max-width: 399px;
      @media (max-width: 991.98px) {
        max-width: 100%;
      }
      @media (min-width: 767.98px) {
        background: linear-gradient(
          179.87deg,
          #e6eaf0 1.02%,
          #e6eaf1 4.79%,
          #e7ebf1 8.76%,
          #eaeef4 14.75%,
          #e8ecf2 19.07%
        );
        color: rgba(0, 0, 52, 0.62);
      }
      @media (max-width: 767.98px) {
        margin: 0;
      }
    }
  }

  // .ref__percent
  &__percent {
    font-weight: 500;
    font-size: 18px;
    line-height: 26px;
    color: rgba(0, 0, 0, 0.62);

    span {
      font-weight: 700;
      color: #000;
      @media (max-width: 767.98px) {
        font-size: 21px;
        line-height: 30px;
        color: #000034;
      }
    }

    @media (max-width: 767.98px) {
      display: inline-flex;
      flex-direction: column;
      font-size: 14px;
      font-weight: 400;
      line-height: 20px;
    }
  }
}
</style>
