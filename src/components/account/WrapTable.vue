<template>
  <div class="wrap wrap-no-padding">
    <div class="wrap_head">
      <main-title tag="h3" v-if="this.title">{{ this.title }} </main-title>
      <span v-else-if="this.viewportWidth < 767.98" class="wrap_head_elem"
        >Отображать в виде</span
      >
      <div class="legend" v-if="this.viewportWidth > 767.98 && this.legend">
        ...
        <div class="legend_elem legend_elem-all">Все: {{ this.all }}</div>
      </div>
    ...
    <router-link class="main__link" v-if="this.link" :to="`/${this.link}`"
...
export default {
  components: { PaymentTable, WorkersTable },
  props: {
    table: Object,
    legend: Boolean,
    title: String,
